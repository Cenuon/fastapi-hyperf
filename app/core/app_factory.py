...
from app.core.auth.session.dependencies import get_current_superuser
from app.core.cache.initialize import close_cache, initialize_cache
from app.core.conf.settings import (
    CacheSettings,
    DatabaseSettings,
    EnvironmentOption,
    EnvironmentSettings,
...
from app.core.db.session import create_tables
from app.core.middleware import ClientCacheMiddleware, SecurityHeadersMiddleware
from app.core.rate_limit.initialize import close_rate_limiter, initialize_rate_limiter
    ...
    **kwargs: Any,
    ...
    if show_docs:
        ...
        async def openapi() -> dict[str, Any]:
            ...
            return _wrap_openapi_schemas(schema)
...
def _wrap_openapi_schemas(openapi_schema: dict[str, Any]) -> dict[str, Any]:
    """Post-process OpenAPI schema to wrap response models in the unified envelope.

    Transforms each JSON response schema so that the Swagger UI shows
    ``{code, message, data}`` instead of the raw model.
    """
    for _path, methods in openapi_schema.get("paths", {}).items():
        for method, operation in methods.items():
            ...
            for status_code, response in operation.get("responses", {}).items():
                ...
                if not schema:
                    continue
                # Wrap the existing schema inside the envelope's "data" field
                json_content["schema"] = {
                    "properties": {
                        "code": {"type": "integer", "example": int(status_code)},
    ...
    # Add PaginatedData schema if referenced
    components = openapi_schema.setdefault("components", {})
    schemas = components.setdefault("schemas", {})
    if "PaginatedData" not in schemas:
